...
export const Wheel = ({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < dataLength; i++) {
      ...
      wheelDataAux[i] = {
        ...data[i],
        style: {
          backgroundColor: data[i].style?.backgroundColor || backgroundColors?.[i % backgroundColors?.length] || DEFAULT_BACKGROUND_COLORS[0],
          ...
          textColor: data[i].style?.textColor || textColors?.[i % textColors?.length] || DEFAULT_TEXT_COLORS[0],
        },
      };
      auxPrizeMap.push([]);
  ...
  useEffect(() => {
    if (mustStartSpinning && !isCurrentlySpinning) {
      ...
      startSpinning();
      const selectedPrize = prizeMap[prizeNumber][Math.floor(Math.random() * prizeMap[prizeNumber]?.length)];
      const finalRotationDegreesCalculated = getRotationDegrees(selectedPrize, getQuantity(prizeMap));
  ...
  return (
    <RouletteContainer style={!isFontLoaded || (totalImages > 0 && loadedImagesCounter !== totalImages) ? { visibility: 'hidden' } : {}}>
      <RotationContainer
      ...
      <RoulettePointerImage style={pointerProps?.style} src={pointerProps?.src || roulettePointer.src} alt="roulette-static" />
    </RouletteContainer>
  );
};
